...
(async () => {
  ...
  if (hasReady) {
    // Second line of defense: explicitly reset animation time to zero. This handles
    // HTML that does not strictly follow the starter tick template
    // (for example, when lastTick uses performance.now() and font-load time gets counted into the first-frame dt).
    ...
    if (seekCorrected) {
      // Wait for two rAFs so the seek takes effect and the t=0 frame renders
