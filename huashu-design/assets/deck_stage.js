 ...
 *
 * Features:
 * - Fixed-size canvas (defaults to 1920x1080) + auto-scale + letterbox
 ...
 * - Speaker notes postMessage support (works with outer renderers)
 ...
 *
 * Speaker notes: add this to `<head>`
...
(function() {
  ...
  class DeckStage extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // Render Shadow DOM first so it does not depend on child parsing timing.
      this._render();

      // Defensive: if the script is in <head> instead of after </deck-stage>,
      // the parser may not have finished child <section> nodes yet, and
      // querySelectorAll would return an empty list. Defer one turn to ensure
      // all child nodes are parsed.
      const init = () => {
        this._collectSlides();
      ...
      if (this.ownerDocument.readyState === 'loading') {
        // The document is still parsing; wait for DOMContentLoaded.
        this.ownerDocument.addEventListener('DOMContentLoaded', init, { once: true });
      } else {
        // The document is already parsed; collect on the next frame.
